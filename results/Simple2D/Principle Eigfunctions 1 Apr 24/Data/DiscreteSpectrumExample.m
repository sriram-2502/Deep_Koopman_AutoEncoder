...
numICs = 2^10; %number of trajectories
...
seed = 1;
X_test = DiscreteSpectrumExampleFn(x1range, x2range, round(2^8), tSpan, mu, lambda, seed);
filename_test = strcat(filenamePrefix, '_test_x.csv');
dlmwrite(filename_test, X_test, 'precision', '%.14f')

seed = 2;
X_val = DiscreteSpectrumExampleFn(x1range, x2range, round(2^8), tSpan, mu, lambda, seed);
